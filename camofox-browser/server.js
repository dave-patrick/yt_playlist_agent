...
async function getSession(userId, { trace = false } = {}) {
  ...
  if (!session) {
    session = await coalesceInflight(sessionCreations, key, async () => {
      ...
      // Inject init script to forcefully and continuously mute all media audio/video elements
      await context.addInitScript(() => {
        ...
        // Run immediately, on intervals, and on play/volumechange events
        forceMute();
        setInterval(forceMute, 250);
        document.addEventListener('play', forceMute, true);
        document.addEventListener('volumechange', forceMute, true);
      }).catch(err => {
        console.error("Failed to add init script for muting audio:", err);
      });
